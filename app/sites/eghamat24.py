...
eghamat24_crawled_datas = []

def eghamat24_process_url(url):
    if "hotels" not in urlparse(url).path.lower():
        return
    try:
    ...
    if response.status_code==200:
        try:
            ...
            eghamat24_crawled_datas.append(hotel)
            print(hotel)
        except Exception as e:
            print('get soap failed.dont worry',e)

def extract_name(soup):
    name_tag = soup.find('h1')
    if not name_tag:
        name_tag = soup.find('div', class_='property-title')
